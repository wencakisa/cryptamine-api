...
RSpec.describe EncryptedMessagesController, type: :controller do
  let!(:rsa) { create(:rsa) }
  let!(:enc_messages) { create_list(:encrypted_message, 5, rsa_id: rsa.id) }
  let(:rsa_id) { rsa.id }
  let(:id) { enc_messages.first.id }

  describe 'GET /rsas/:rsa_id/encrypted_messages' do
  ...
  describe 'GET /rsas/:rsa_id/encrypted_messages/:id' do
    ...
    context 'when rsa enecrypted message does not exist' do
      ...
      it 'returns a not found message' do
        expect(response.body).to match(/Couldn't find EncryptedMessage/)
      end
    end
  end

  describe 'POST /rsas/:rsa_id/encrypted_messages' do
